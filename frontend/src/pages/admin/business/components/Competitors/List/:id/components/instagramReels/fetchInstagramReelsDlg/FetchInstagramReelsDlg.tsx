...
function FetchInstagramReelsDlg({ open, onClose }: any) {
  const dispatch = useAppDispatch();
  const { id } = useParams<{ id: string }>();
  const [ fetchInstagramReels, { isLoading: isLoadingReels } ] = useFetchInstagramReelsMutation();
  ...
  const getPostsData = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response: ApiResponse<any> = await fetchInstagramReels({
        id,
        form: {
...
export default FetchInstagramReelsDlg;
