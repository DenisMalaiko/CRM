...
import FetchInstagramReelsDlg from './FetchInstagramReelsDlg';
...
const mockFetchInstagramReels = jest.fn();
const mockGetInstagramReels = jest.fn();

let mockIsLoadingReels = false;

jest.mock('../../../../../../../../../../store/competitor/competitorApi', () => ({
  useFetchInstagramReelsMutation: () => [mockFetchInstagramReels, { isLoading: mockIsLoadingReels }],
...
function renderDialog(props: { open?: boolean; onClose?: () => void } = {}) {
  ...
  return render(<FetchInstagramReelsDlg {...defaults} {...props} />);
}

// ── Tests ─────────────────────────────────────────────────────────────────────

describe('FetchInstagramReelsDlg', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockIsLoadingReels = false;
    mockFetchInstagramReels.mockReturnValue({
  ...
  describe('form submission', () => {
    it('calls fetchInstagramReels with the competitor id and form data on submit', async () => {
      renderDialog();

      userEvent.click(screen.getByRole('button', { name: /^get$/i }));

      await waitFor(() => {
        expect(mockFetchInstagramReels).toHaveBeenCalledWith(
          expect.objectContaining({ id: 'competitor-1' })
        );
      });
    });

    it('fetches updated reels and dispatches setInstagramReels after successful submission', async () => {
      const freshReels = [{ id: 'r-1', text: 'New reel' }];
      mockGetInstagramReels.mockReturnValue({
        unwrap: () => Promise.resolve({ data: freshReels }),
      });
    ...
    it('does not call onClose when the API returns no data', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchInstagramReels).toHaveBeenCalled();
      });
