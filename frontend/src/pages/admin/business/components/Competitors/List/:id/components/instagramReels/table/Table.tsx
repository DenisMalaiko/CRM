...
import { useGetInstagramReelsMutation } from "../../../../../../../../../../store/competitor/competitorApi";
import { setInstagramReels } from "../../../../../../../../../../store/competitor/competitorSlice";

// Components
import TextDlg from "../../../../../../../../../../components/textDlg/TextDlg";
import SliderDlg from "../../../../../../../../../../components/sliderDlg/SliderDlg";
import FetchInstagramReelsDlg from "../fetchInstagramReelsDlg/FetchInstagramReelsDlg";
...
function InstagramReelsTable() {
  ...
  const { instagramReels } = useAppSelector((state) => state.competitorModule);

  useEffect(() => {
    const fetchData = async () => {
      try {
        if(id) {
          const responsePosts: ApiResponse<any> = await getInstagramReels(id).unwrap();

          if(responsePosts && responsePosts.data) dispatch(setInstagramReels(responsePosts.data));
        }
      } catch (error) {
        showError(error);
      }
    ...
    fetchData();
  }, [dispatch, id, getInstagramReels]);

  const sortedPosts = useMemo(() => {
    if (!instagramReels?.length) return [];

    return [...instagramReels].sort((a, b) => {
      if (sortKey === 'postedAt') {
  ...
  }, [instagramReels, sortKey, sortDir]);
  ...
  return (
    <div className="w-full rounded-2xl bg-white shadow border border-slate-200 mb-5">
      <div className="w-full flex items-center border-b p-4 justify-between">
        <h2 className="text-lg text-left font-semibold text-slate-800">Instagram Reels</h2>
...
export default InstagramReelsTable;
